...
pc_reweight = np.loadtxt("pc_reweight.txt")
img_reweight = np.loadtxt("img_reweight.txt")
...
for i in range(len(DATABASE_PC_VECTORS)):
	reweight = np.tile(pc_reweight,(DATABASE_PC_VECTORS[i].shape[0],1))
	DATABASE_PC_VECTORS[i] = DATABASE_PC_VECTORS[i]*reweight
for i in range(len(QUERY_PC_VECTORS)):
	reweight = np.tile(pc_reweight,(QUERY_PC_VECTORS[i].shape[0],1))
	QUERY_PC_VECTORS[i] = QUERY_PC_VECTORS[i]*reweight
...
for i in range(len(DATABASE_IMG_VECTORS)):
	reweight = np.tile(img_reweight,(DATABASE_IMG_VECTORS[i].shape[0],1))
	DATABASE_IMG_VECTORS[i] = DATABASE_IMG_VECTORS[i]*reweight
for i in range(len(QUERY_IMG_VECTORS)):
	reweight = np.tile(img_reweight,(QUERY_IMG_VECTORS[i].shape[0],1))
	QUERY_IMG_VECTORS[i] = QUERY_IMG_VECTORS[i]*reweight
