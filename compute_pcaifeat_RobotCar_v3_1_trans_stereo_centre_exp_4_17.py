...
PC_MODEL_PATH = ""
IMG_MODEL_PATH = ""
MODEL_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_4_17/model_00441147.ckpt"
...
def init_all_feat():
	if TRAINING_MODE != 2:
		pc_feat = np.empty([0,128],dtype=np.float32)
	if TRAINING_MODE != 1:
		img_feat = np.empty([0,128],dtype=np.float32)
	if TRAINING_MODE == 3:
