...
DATABASE_VECTORS_FILENAME = "database_img_feat_00882294.pickle"
QUERY_VECTORS_FILENAME = "query_img_feat_00882294.pickle"
#result output
...
def main():
	...
	for m in range(len(QUERY_SETS)):
		...
		for n in range(len(QUERY_SETS)):
			...
			m_seq_recall.append(pair_opr)
			
			for x in pair_similarity:
				similarity.append(x)
		
		seq_recall.append(np.mean(m_seq_recall))
		seq_size.append(len(m_seq_recall))
	...
	one_percent_recall = np.array(one_percent_recall)
	np.savetxt('one_percent_recall.txt',one_percent_recall,fmt='%.3f',delimiter=',')
	
	seq_recall = np.array(seq_recall)
	np.savetxt('seq_recall.txt',seq_recall,fmt='%.3f',delimiter=',')
	
	seq_size = np.array(seq_size)
	np.savetxt('seq_size.txt',seq_size,fmt='%d',delimiter=',')
