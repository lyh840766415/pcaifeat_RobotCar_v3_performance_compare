...
def train_one_epoch(sess, ops, train_writer, test_writer, epoch, saver):
    ...
    num_to_take=10
    if(num_to_take > NEGATIVES_PER_QUERY):
    ...
    while True:
        ...
        for i in range(BATCH_NUM_QUERIES):
        	while len(batch_keys) < BATCH_NUM_QUERIES:
        		...
        		filename = os.path.join(BASE_PATH,"%s_mono_left.png"%(TRAINING_QUERIES[cur_key]["query"][:-4]))
        		...
        		for i in range(len(TRAINING_QUERIES[cur_key]["positives"])):
        			filename =  os.path.join(BASE_PATH,"%s_mono_left.png"%(TRAINING_QUERIES[TRAINING_QUERIES[cur_key]["positives"][i]]["query"][:-4]))
        			if not os.path.exists(filename):
