...
parser.add_argument('--log_dir', default='log_hard_mine/', help='Log dir [default: log]')
...
parser.add_argument('--negatives_per_query', type=int, default=5, help='Number of definite negatives in each training tuple [default: 18]')
parser.add_argument('--max_epoch', type=int, default=15, help='Epoch to run [default: 20]')
...
def get_correspond_img(pc_filename):
	pc_path = '/data/lyh/benchmark_datasets'
	pc_filename = os.path.join(pc_path,pc_filename)
	#print(pc_filename)
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        # Restore a model
        saver.restore(sess, os.path.join(LOG_DIR, "model_00051101.ckpt"))
...
def train_one_epoch(sess, ops, train_writer, test_writer, epoch, saver):
    ...
    num_to_take=NEGATIVES_PER_QUERY
    ...
    while True:
        batch_keys = []
        for i in range(BATCH_NUM_QUERIES):
        	while len(batch_keys) < BATCH_NUM_QUERIES:
        		if CUR_LOAD >= len(TRAINING_QUERIES.keys()):
        			return
        		
        		cur_key = train_file_idxs[CUR_LOAD]
        		#print(TRAINING_QUERIES[cur_key]["query"])
        		if len(TRAINING_QUERIES[cur_key]["positives"]) < POSITIVES_PER_QUERY:
        			CUR_LOAD = CUR_LOAD + 1
        			continue
        		...
        		if valid_pos < POSITIVES_PER_QUERY:
        			CUR_LOAD = CUR_LOAD + 1
        			continue
