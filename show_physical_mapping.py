...
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_19"
...
def train_one_step(sess,ops,train_feed_dict,train_writer,is_training = True):
	...
	if TRAINING_MODE == 3:
		if ONLY_TRAIN_FUSION:
			...
			summary,step,all_loss,_= sess.run([ops["merged"],ops["step"],ops["all_loss"],ops["img_train_op"]],feed_dict = train_feed_dict)
			#print("batch num = %d , all_loss = %f"%(step, all_loss))
...
def get_trans_datas(load_pc_filenames,img_data,pc_data,pool):
	dict_list = []
	for i in range(pc_data.shape[0]):
		para = ("%s_imgpos.txt"%(load_pc_filenames[i][:-4]),pc_data[i,:,:],img_data[i,:,:,:])
		#print(para_dict)
...
def load_data(train_file_idxs):
	...
	while True:
		...
		if (cnt*LOAD_FEAT_RATIO)%300 == 0:
			...
			trans_data = None
			if TRAINING_MODE != 2:
				trans_data = get_trans_datas(eval_pc_filenames,img_data,pc_data,pool)
			...
			trans_data = None
			if TRAINING_MODE != 2:
				trans_data = get_trans_datas(load_pc_filenames,img_data,pc_data,pool)
				for i in range(len(pc_data)):
