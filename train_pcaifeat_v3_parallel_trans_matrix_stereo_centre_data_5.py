...
def init_imgnetwork(pc_trans_feat):
	with tf.variable_scope("img_var"):
		img_placeholder = tf.placeholder(tf.float32,shape=[FEAT_BARCH_SIZE*BATCH_DATA_SIZE,240,320,3])
...
def init_pcnetwork(step):
	with tf.variable_scope("pc_var"):
		...
		trans_mat_placeholder = tf.placeholder(tf.float32,shape=[FEAT_BARCH_SIZE*BATCH_DATA_SIZE,80,4096])
