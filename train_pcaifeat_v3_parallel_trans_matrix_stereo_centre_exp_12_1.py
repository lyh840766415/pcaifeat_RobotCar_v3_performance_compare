...
RAND_INIT = False
# model path
MODEL_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_12/model_01320440.ckpt"
...
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_12_1"
...
def init_fusion_network(pc_feat,img_feat,img_pc_feat):
	with tf.variable_scope("fusion_var"):
		concat_feat = tf.concat((pc_feat,img_feat,img_pc_feat),axis=1)
		pcai_feat = tf.layers.dense(concat_feat,EMBBED_SIZE,activation=tf.nn.relu,name='dense_global_fuse_1')
		pcai_feat = tf.layers.dense(pcai_feat,EMBBED_SIZE,activation=tf.nn.relu,name='dense_global_fuse_2')
		pcai_feat = tf.layers.dense(pcai_feat,EMBBED_SIZE,activation=tf.nn.relu,name='dense_global_fuse_3')
...
def init_pcainetwork():
	...
	if TRAINING_MODE == 3:
		pcai_feat = init_fusion_network(pc_feat,img_feat,img_pc_feat)
	...
	global_fusion_variable = [v for v in variables if v.name.split('/')[0] =='fusion_var']
...
def init_train_saver():	
	...
	all_variable = [v for v in variables if not (v.name.split('/')[0] =='fusion_var')]
...
def train_one_step(sess,ops,train_feed_dict,train_writer,is_training = True):
	...
	if TRAINING_MODE == 3:
		if ONLY_TRAIN_FUSION:
			...
			summary,step,all_loss,_,_,_= sess.run([ops["merged"],ops["step"],ops["all_loss"],ops["resnet_train_op"],ops["img_train_op"],ops["fusion_train_op"]],feed_dict = train_feed_dict)
			#print("batch num = %d , all_loss = %f"%(step, all_loss))
