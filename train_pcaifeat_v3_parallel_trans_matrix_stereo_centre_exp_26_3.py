...
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_26_3"
...
def init_pcnetwork(step):
	with tf.variable_scope("pc_var"):
		...
		pc_feat = pointnetvlad(pc_placeholder,is_training_pl,bn_decay,True)	
...
def init_pcainetwork():
	...
	pc_variable = [v for v in variables if v.name.split('/')[0] =='pc_var' and v.name.split('/')[1] != 'non_local']
	non_local_variable = [v for v in variables if v.name.split('/')[0] =='pc_var' and v.name.split('/')[1] == 'non_local']
	...
	for var in non_local_variable:
		print(var)
	
	print("nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn")
	exit()
	'''
	...
	with tf.control_dependencies(update_ops):
		if TRAINING_MODE != 2:
			pc_train_op = optimizer.minimize(pc_loss, global_step=step, var_list=non_local_variable)
		if TRAINING_MODE != 1:
...
def train_one_step(sess,ops,train_feed_dict,train_writer,is_training = True):
	...
	if TRAINING_MODE == 3:
		if ONLY_TRAIN_FUSION:
			...
			summary,step,all_loss,_,_,_,_= sess.run([ops["merged"],ops["step"],ops["all_loss"],ops["pc_train_op"],ops["resnet_train_op"],ops["img_train_op"],ops["fusion_train_op"]],feed_dict = train_feed_dict)
			#print("batch num = %d , all_loss = %f"%(step, all_loss))
...
def model_save(sess,step,train_saver):
	if TRAINING_MODE == 1:
		save_path = train_saver['all_saver'].save(sess,os.path.join(LOG_PATH, "pc_model_%08d.ckpt"%(step)))
