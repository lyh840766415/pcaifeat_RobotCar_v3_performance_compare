...
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_27"
...
def init_pcainetwork():
	...
	with tf.control_dependencies(update_ops):
		if TRAINING_MODE != 2:
			pc_train_op = optimizer.minimize(pc_loss, global_step=step, var_list=pc_variable+local_fusion_variable+vlad_variable)
		if TRAINING_MODE != 1:
			img_train_op = optimizer.minimize(img_loss, global_step=step, var_list=resnet_variable+local_fusion_variable)
		if TRAINING_MODE == 3:
			all_train_op = optimizer.minimize(all_loss, global_step=step)
			fusion_train_op = optimizer.minimize(finally_loss, global_step=step)
...
def train_one_step(sess,ops,train_feed_dict,train_writer,is_training = True):
	...
	if TRAINING_MODE == 3:
		if ONLY_TRAIN_FUSION:
			...
			summary,step,all_loss,_,_,_= sess.run([ops["merged"],ops["step"],ops["all_loss"],ops["pc_train_op"],ops["img_train_op"],ops["fusion_train_op"]],feed_dict = train_feed_dict)
			#print("batch num = %d , all_loss = %f"%(step, all_loss))
