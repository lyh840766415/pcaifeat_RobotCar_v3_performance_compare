...
import nets_v3.resnet_v1_trans_no_fc_1000d_max_pool as resnet
import shutil
...
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_29_2"
...
def train_one_step(sess,ops,train_feed_dict,train_writer,is_training = True):
	...
	if TRAINING_MODE == 3:
		if ONLY_TRAIN_FUSION:
			...
			summary,step,all_loss,_,_,_,_= sess.run([ops["merged"],ops["step"],ops["pc_loss"],ops["img_train_op"],ops["pc_train_op"],ops["pc_img_train_op"],ops["img_pc_train_op"]],feed_dict = train_feed_dict)
			#print("batch num = %d , all_loss = %f"%(step, all_loss))
