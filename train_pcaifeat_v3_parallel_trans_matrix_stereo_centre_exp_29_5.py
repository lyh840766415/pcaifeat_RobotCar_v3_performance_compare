...
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_29_5"
...
def init_pcainetwork():
	...
	if TRAINING_MODE != 1:
		img_placeholder, img_feat = init_imgnetwork(pc_trans_feat)
	if TRAINING_MODE == 3:
		pcai_feat = None
	
	#prepare data and loss
	if TRAINING_MODE == 3:
		pc_img_sim_loss = tf.nn.l2_loss(pc_feat-img_feat)/BATCH_DATA_SIZE
		tf.summary.scalar('pc_img_sim_loss', pc_img_sim_loss)
		...
		pc_img_loss = lazy_quadruplet_loss(q_pc_vec, pos_img_vec, neg_img_vec, oth_img_vec, MARGIN1, MARGIN2)
		tf.summary.scalar('pc_img_loss', pc_img_loss)
		img_pc_loss = lazy_quadruplet_loss(q_img_vec, pos_pc_vec, neg_pc_vec, oth_pc_vec, MARGIN1, MARGIN2)
		tf.summary.scalar('img_pc_loss', img_pc_loss)
	...
	with tf.control_dependencies(update_ops):
		...
		if TRAINING_MODE != 1:
			img_train_op = optimizer.minimize(img_loss, global_step=step, var_list=img_variable)
		if TRAINING_MODE == 3:
			pc_img_train_op = optimizer.minimize(pc_img_loss, global_step=step, var_list=pc_variable)
			img_pc_train_op = optimizer.minimize(img_pc_loss, global_step=step, var_list=img_variable)
			pc_img_sim_train_op = optimizer.minimize(pc_img_sim_loss, global_step=step, var_list=img_variable)
	...
	if TRAINING_MODE == 3 and ONLY_TRAIN_FUSION:
		ops = {
			...
			"img_pc_train_op":img_pc_train_op,
			"merged":merged,
			"step":step}
		return ops
	...
	if TRAINING_MODE == 3:
		ops = {
			...
			"pc_img_sim_train_op":pc_img_sim_train_op,
			"step":step}
		return ops
...
def train_one_step(sess,ops,train_feed_dict,train_writer,is_training = True):
	...
	if TRAINING_MODE == 3:
		if ONLY_TRAIN_FUSION:
			...
			summary,step,all_loss,_,_= sess.run([ops["merged"],ops["step"],ops["pc_loss"],ops["pc_train_op"],ops["pc_img_sim_train_op"]],feed_dict = train_feed_dict)
			#print("batch num = %d , all_loss = %f"%(step, all_loss))
