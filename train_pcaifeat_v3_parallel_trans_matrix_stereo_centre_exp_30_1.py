...
import pointnetvlad_v3.img_and_learned_center_loupe as lp
...
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_30_1"
...
def init_imgnetwork():
	with tf.variable_scope("img_var"):
		...
		imgraw_feat, img_feat_after_pooling, body_prefix = resnet.endpoints(img_placeholder,is_training=True)
...
def init_fusion_network(pc_feat,pointnet_raw_feat,img_feat,imgraw_feat,is_training=True):
...
def init_pcainetwork():
	...
	if TRAINING_MODE == 3:
		...
		all_loss = pc_loss*2+img_loss*2+finally_loss
