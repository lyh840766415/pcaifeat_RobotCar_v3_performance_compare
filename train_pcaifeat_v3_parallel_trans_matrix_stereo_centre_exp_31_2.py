...
from pointnetvlad_v3.pointnetvlad_specify_outdim_cls import *
...
RAND_INIT = False
# model path
MODEL_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_31_2/model_00441147.ckpt"
PC_MODEL_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_31_5_1/pc_model_00441147.ckpt"
IMG_MODEL_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_31_4/img_model_00441147.ckpt"
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_31_2_1"
...
def init_imgnetwork():
	with tf.variable_scope("img_var"):
		...
		img_dense = tf.get_variable("dense",[2048, 256],initializer=tf.random_normal_initializer(stddev=1 / math.sqrt(64)))
		
		img_feat = tf.matmul(img_feat_after_pooling, img_dense)
...
def init_pcnetwork(step):
	with tf.variable_scope("pc_var"):
		...
		pc_feat = pointnetvlad(pc_placeholder,is_training_pl,bn_decay,256)
...
def init_fusion_network(pc_feat,img_feat,is_training=True):
	with tf.variable_scope("fusion_var"):
		pc_feat = slim.batch_norm(pc_feat,center=True,scale=True,is_training=is_training,scope="pc_bn_0")
		img_feat = slim.batch_norm(img_feat,center=True,scale=True,is_training=is_training,scope="img_bn_0")
		
		img_trans = tf.get_variable("img_trans",[256, 256],initializer=tf.random_normal_initializer(stddev=1 / math.sqrt(64)))
		img_feat = tf.matmul(img_feat, img_trans)
		
		pc_trans = tf.get_variable("pc_trans",[256, 256],initializer=tf.random_normal_initializer(stddev=1 / math.sqrt(64)))
		pc_feat = tf.matmul(pc_feat, pc_trans)
		
		pc_feat = slim.batch_norm(pc_feat,center=True,scale=True,is_training=is_training,scope="pc_bn_1")
		img_feat = slim.batch_norm(img_feat,center=True,scale=True,is_training=is_training,scope="img_bn_1")
		
		img_sub_pc = img_feat - pc_feat
		pc_sub_img = pc_feat - img_feat
		
		img_sub_pc = slim.batch_norm(img_sub_pc,center=True,scale=True,is_training=is_training,scope="img_sub_pc_bn")
		pc_sub_img = slim.batch_norm(pc_sub_img,center=True,scale=True,is_training=is_training,scope="pc_sub_img_bn")
		
		pcai_feat = tf.concat((pc_feat,img_feat,img_sub_pc,pc_sub_img),axis=1)
		
		input_dim = pcai_feat.get_shape().as_list()[1] 
		reweights = tf.get_variable("reweights",[input_dim, input_dim],initializer = tf.random_normal_initializer(stddev=1 / math.sqrt(input_dim)))	
		
		gates = tf.matmul(pcai_feat, reweights)
...
def init_train_saver():
	...
	pc_load_variable = [v for v in variables if v.name.split('/')[0] =='pc_var' and v.name.split('/')[1].split(':')[0] != "hidden1_weights"]
	img_load_variable = [v for v in variables if v.name.split('/')[0] =='img_var' and v.name.split('/')[1].split(':')[0] != "dense"]
	
	'''
	for var in img_load_variable:
		print(var)
	'''
	...
	if TRAINING_MODE != 2:
		...
		pc_load_saver = tf.train.Saver(pc_load_variable)
	if TRAINING_MODE != 1:
		img_saver = tf.train.Saver(img_variable)
		img_load_saver = tf.train.Saver(img_load_variable)
	...
	train_saver = {
		...
		'pc_saver':pc_saver,
		'img_saver':img_saver,
		'img_load_saver':img_load_saver,
		'pc_load_saver':pc_load_saver}
