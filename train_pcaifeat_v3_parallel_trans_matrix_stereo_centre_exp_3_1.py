...
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_3_1"
...
def init_imgnetwork():
	with tf.variable_scope("img_var"):
		...
		resnet_feat,resnetvlad_feat = resnet.endpoints(img_placeholder,is_training=True)
		resnetvlad_feat = tf.layers.dense(resnetvlad_feat, EMBBED_SIZE,activation=tf.nn.relu)
	return img_placeholder,resnet_feat,resnetvlad_feat
...
def init_pcnetwork(step):
	with tf.variable_scope("pc_var"):
		...
		pc_feat_after_shape = pointnetvlad(pc_placeholder,is_training_pl,bn_decay)
...
def init_pcainetwork():
	...
	if TRAINING_MODE != 1:
		img_placeholder, resnet_feat,resnetvlad_feat = init_imgnetwork()
	if TRAINING_MODE == 3:
		pcai_feat = init_fusion_network(pc_feat,resnetvlad_feat)
	...
	if TRAINING_MODE != 1:
		resnetvlad_feat = tf.reshape(resnetvlad_feat,[FEAT_BATCH_SIZE,BATCH_DATA_SIZE,resnetvlad_feat.shape[1]])
		q_img_vec, pos_img_vec, neg_img_vec, oth_img_vec = tf.split(resnetvlad_feat, [1,POS_NUM,NEG_NUM,OTH_NUM],1)
		resnetvlad_loss = lazy_quadruplet_loss(q_img_vec, pos_img_vec, neg_img_vec, oth_img_vec, MARGIN1, MARGIN2)
		tf.summary.scalar('resnetvlad_loss', resnetvlad_loss)
		
		resnet_feat = tf.reshape(resnet_feat,[FEAT_BATCH_SIZE,BATCH_DATA_SIZE,resnet_feat.shape[1]])
		q_img_vec, pos_img_vec, neg_img_vec, oth_img_vec = tf.split(resnet_feat, [1,POS_NUM,NEG_NUM,OTH_NUM],1)
	...
	if TRAINING_MODE == 3:
		...
		all_loss = pc_loss*2+resnetvlad_loss+finally_loss
	...
	vlad_variable = [v for v in variables if v.name.split('/')[0] =='img_var' and v.name.split('/')[1] != 'resnet_v1_50']
	all_variable = resnet_variable + vlad_variable
	...
	with tf.control_dependencies(update_ops):
		...
		if TRAINING_MODE != 1:
			...
			resnetvlad_train_op = optimizer.minimize(resnetvlad_loss, global_step=step, var_list = vlad_variable)
		if TRAINING_MODE == 3:
	...
	if TRAINING_MODE == 2:
		ops = {
			...
			"resnetvlad_loss":resnetvlad_loss,	
			"resnet_train_op":resnet_train_op,
			"resnetvlad_train_op":resnetvlad_train_op,
			"merged":merged,
			"step":step}
		return ops
...
def train_one_step(sess,ops,train_feed_dict,train_writer,is_training = True):
	global STEP
	if not is_training:
		summary,step,all_loss = sess.run([ops["merged"],ops["step"],ops["resnetvlad_loss"]],feed_dict = train_feed_dict)
	...
	if TRAINING_MODE == 2:
		summary,step,all_loss,_,_,= sess.run([ops["merged"],ops["step"],ops["resnetvlad_loss"],ops["resnet_train_op"],ops["resnetvlad_train_op"]],feed_dict = train_feed_dict)
