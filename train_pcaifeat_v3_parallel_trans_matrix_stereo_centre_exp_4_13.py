...
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_4_13"
...
def init_pcainetwork():
	...
	with tf.control_dependencies(update_ops):
		...
		if TRAINING_MODE == 3:
			...
			fusion_train_op = optimizer.minimize(finally_loss, global_step=step,var_list=fusion_variable+img_train_variable)
			all_train_op = optimizer.minimize(2*pc_loss+2*img_loss+finally_loss, global_step=step)
...
def get_load_batch_filename(load_batch_keys,quadruplet,all_hard_negs = None):	
	pc_files = []
	img_files = []
	for key_cnt ,key in enumerate(load_batch_keys):
		...
		hard_negs = all_hard_negs[key_cnt]
		#for hard minming
		if(len(hard_negs)==0):			
			for i in range(NEG_NUM):
				while True:
			...
			for i in range(len(hard_negs)):
				neg_indices.append(hard_negs[i])
				pc_files.append(TRAINING_QUERIES[hard_negs[i]]["query"])
				filename = "%s_stereo_centre.png"%(TRAINING_QUERIES[hard_negs[i]]["query"][:-4])
				img_files.append(filename)
				if(len(neg_indices) >= NEG_NUM):
					break
			
			while(len(neg_indices) < NEG_NUM):
				while True:
					neg_ind = random.randint(0,len(TRAINING_QUERIES.keys())-1)
					if is_negative(neg_ind,TRAINING_QUERIES[key]["not_negative"]):
						break
				...
				filename = "%s_stereo_centre.png"%(TRAINING_QUERIES[neg_ind]["query"][:-4])
				if filename in img_files[BATCH_DATA_SIZE*(key_cnt)+1+POS_NUM:BATCH_DATA_SIZE*(key_cnt)+1+POS_NUM+len(neg_indices)]:
					continue
				if os.path.exists(filename):
					break
...
def compute_latent_feat(pc_data,img_data,ops,sess):
	global EP
	global BATCH_DATA_SIZE
	
	input_data_size = pc_data.shape[0]
	
	remind_index = pc_data.shape[0]%BATCH_DATA_SIZE
	tot_batches = pc_data.shape[0]//BATCH_DATA_SIZE
	fake_pc = np.zeros([BATCH_DATA_SIZE - remind_index,pc_data.shape[1],pc_data.shape[2]],dtype=np.float32)
	fake_img = np.zeros([BATCH_DATA_SIZE - remind_index,img_data.shape[1],img_data.shape[2],img_data.shape[3]],dtype=np.float32)
	pc_data = np.concatenate((pc_data,fake_pc),axis=0)
	img_data = np.concatenate((img_data,fake_img),axis=0)
	
	
	pcai_feat = np.empty([0,3048],dtype=np.float32)
	for i in range(pc_data.shape[0]//BATCH_DATA_SIZE):
		train_feed_dict = prepare_batch_data(pc_data,img_data,i,ops,EP)
		cur_pcai_feat= sess.run([ops["pcai_feat"]],feed_dict = train_feed_dict)
		cur_pcai_feat = cur_pcai_feat[0].reshape(-1,cur_pcai_feat[0].shape[-1])
		pcai_feat = np.concatenate((pcai_feat,cur_pcai_feat),axis=0)
		
	
	pcai_feat = pcai_feat[0:input_data_size,:]
...
#return hard neg id
def get_hard_negative(load_batch_keys,ops,sess,num_to_take=NEG_NUM):
	...
	#load data from disk
	pc_data,img_data = load_img_pc(pc_files,img_files,pool,False)
	
	#compute query latern vector for all keys
	query_feat = compute_latent_feat(pc_data,img_data,ops,sess)
	
	for key_cnt,key in enumerate(load_batch_keys):
		#sample 4000 neg
		random_negs = []
		while(len(random_negs)<4000):
			neg_ind = random.randint(0,len(TEST_QUERIES.keys())-1)
			if is_negative(neg_ind,TRAINING_QUERIES[key]["not_negative"]):
				random_negs.append(neg_ind)
		
		hard_neg = get_random_hard_negatives(query_feat[key_cnt,:],random_negs,num_to_take)
		hard_neg_all.append(hard_neg)
...
def load_data(train_file_idxs,ops,sess):
	...
	while True:
		...
		if (cnt*LOAD_FEAT_RATIO)%300 == 0:
			...
			hard_negs = get_hard_negative(load_batch_keys,ops,sess)
						
			#select load_batch tuple
			load_pc_filenames,load_img_filenames = get_load_batch_filename(load_batch_keys,quadruplet,hard_negs)
...
def get_latent_vectors(sess, ops):
	...
	latent_batch_size = BATCH_DATA_SIZE*10
	
	remind_index = len(TRAINING_QUERIES.keys())%latent_batch_size
	tot_batches = len(TRAINING_QUERIES.keys())//latent_batch_size
	
	for i in range(tot_batches+1):
		pc_files = []
		img_files = []
		for j in range(latent_batch_size):
			cur_ind = i*latent_batch_size+j
			if cur_ind >= len(TRAINING_QUERIES.keys()):
				cur_ind = len(TRAINING_QUERIES.keys()) - 1
				
			pc_files.append(TRAINING_QUERIES[cur_ind]["query"])
			img_files.append("%s_stereo_centre.png"%(TRAINING_QUERIES[cur_ind]["query"][:-4]))
		
		pc_data,img_data = load_img_pc(pc_files,img_files,pool,False)
		cur_pcai_feat = compute_latent_feat(pc_data,img_data,ops,sess)
		pcai_feat = np.concatenate((pcai_feat,cur_pcai_feat),axis=0)
	
	pcai_feat = pcai_feat[0:len(TRAINING_QUERIES.keys()),:]
	return pcai_feat
...
def main():
	...
	with tf.Session(config=config) as sess:
		...
		#update latent_vector
		
		TRAINING_LATENT_VECTORS = get_latent_vectors(sess, ops)
		print(len(TRAINING_LATENT_VECTORS))
		...
		for ep in range(EPOCH):
			...
			CUR_LOAD = 0
			#load data thread
			load_data_thread = threading.Thread(target=load_data, args=(train_file_idxs,ops,sess))
