...
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_4_14"
...
def init_fusion_network(pc_feat,img_feat,is_training=True):
	with tf.variable_scope("fusion_var"):
		...
		pcai_dense = tf.get_variable("pcai_dense",[3048, 256],initializer=tf.random_normal_initializer(stddev=1 / math.sqrt(256)))
		
		pcai_feat = tf.matmul(pcai_feat, pcai_dense)
...
def init_network_variable(sess,train_saver):
	...
	if TRAINING_MODE == 3:
		train_saver['all_load_saver'].restore(sess,MODEL_PATH)
...
def init_train_saver():
	...
	except_fusion_variable = [v for v in variables if v.name.split('/')[0] !='fusion_var']
	all_load_saver = tf.train.Saver(except_fusion_variable)
