...
RAND_INIT = False
# model path
MODEL_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_4_4/model_00441147.ckpt"
PC_MODEL_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_4_16/pc_model_00552184.ckpt"
IMG_MODEL_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_4_15/img_model_00441147.ckpt"
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_4_17"
...
def init_imgnetwork(is_training = True):
	with tf.variable_scope("img_var"):
		...
		img_dense = tf.get_variable("dense",[2048, EMBBED_SIZE],initializer=tf.random_normal_initializer(stddev=1 / math.sqrt(EMBBED_SIZE)))
		img_feat_after_pooling = tf.matmul(img_feat_after_pooling, img_dense)
...
def init_pcnetwork(step):
	with tf.variable_scope("pc_var"):
		...
		pc_feat = pointnetvlad(pc_placeholder,is_training_pl,bn_decay,EMBBED_SIZE)
...
def init_train_saver():
	...
	img_variable = [v for v in variables if v.name.split('/')[0] =='img_var' or v.name.split('/')[0] == 'img_reweight']
