...
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_performance_compare/log/train_save_trans_exp_6_5"
...
def init_pcainetwork():
	...
	if TRAINING_MODE != 1:
		img_pc_feat = tf.reshape(img_feat,[FEAT_BATCH_SIZE,BATCH_DATA_SIZE,img_feat.shape[1]])
...
def train_one_step(sess,ops,train_feed_dict,train_writer,is_training = True):
	if not is_training:
		summary,step,all_loss = sess.run([ops["merged"],ops["step"],ops["resnet_loss"]],feed_dict = train_feed_dict)
	...
	if TRAINING_MODE == 2:
		summary,step,all_loss,_,= sess.run([ops["merged"],ops["step"],ops["resnet_loss"],ops["resnet_train_op"]],feed_dict = train_feed_dict)
